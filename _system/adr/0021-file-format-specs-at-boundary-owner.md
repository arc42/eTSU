...
[[GLO-010-entity-c|Entity C]]/locations, [[GLO-023-entity-d|Entity D catalog]]) are
...
- **At the boundary where the data crosses the system edge** (path A) — chosen.

The format proposals already drafted in `raw/drafts/` (`entity-d-format-proposal.md`,
`entity-c-format-proposal.md`, `entity-a-format-proposal.md`,
`entity-b-format-proposal.md`) all describe *imports* — i.e. flows that cross the system
...
2. **External feeds** — recurring data crossing an edge to/from an external *system* — carry
   ...
   relevant `flows:` entry. The Entity D catalog import (System X, yearly) is specified on
   [[EIF-001-system-x-catalog-feed]].
3. **Internal configuration imports** — bootstrapping files loaded at program start, with no
   external partner system — are specified on the [[FR-006-system-administration]]
   bootstrapping requirement (the boundary owner is the admin/deployment action, not an EIF).
   The three CSV formats for Entity C, Entity A, and Entity B live there.
...
5. **Every format spec carries a fictive example** *(added 2026-06-04)* — a fenced
   ...
   exercising the edge cases the validation rules name (encoding of non-ASCII characters,
   optional columns such as Entity C's `phone`/`email` "and/or"). The example is part of the
...
- Follow-up work (separate to-dos): create the FR-006 config-import story/stories and anchor
  the Entity C format there; bring `entity-a-format-proposal.md` and
  `entity-b-format-proposal.md` onto CSV, grill, and ingest them likewise; DM-001/003/006
