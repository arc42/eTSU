---
id: STK-001
type: stakeholder
title: Gus Renoir
status: draft
created: 2026-09-06
updated: 2026-09-09
sources:
  - "[[SRC-002-georg-mayrhauser-vision-goals-stakeholders]]"
related:
  - "[[GOAL-001-eTSU-vision]]"
  - "[[GOAL-002-global-market-expansion]]"
  - "[[GOAL-003-operational-excellence]]"
  - "[[GOAL-005-frictionless-global-art-trade]]"
  - "[[STK-012-creative-genius]]"
  - "[[GLO-004-tsu]]"
  - "[[FR-001-view-piece-inventory-and-status]]"
  - "[[FR-002-settle-finances-and-legal-obligations]]"
  - "[[FR-006-tour-visibility]]"
  - "[[FR-007-settlement-and-compliance]]"
  - "[[GOAL-006-settlement-correct-and-on-time]]"
  - "[[FR-008-collect-the-purchase-deposit]]"
  - "[[FR-009-collect-the-purchase-balance]]"
  - "[[FR-010-settle-a-failed-purchase]]"
  - "[[FR-011-pay-artist-commission]]"
  - "[[FR-012-settle-a-loss-claim-on-a-sold-piece]]"
  - "[[FR-017-artwork-inventory]]"
  - "[[FR-019-create-the-piece-inventory]]"
  - "[[FR-020-add-piece-details]]"
  - "[[FR-021-upload-piece-images]]"
  - "[[FR-022-display-current-piece-information]]"
  - "[[FR-023-modify-piece-location]]"
  - "[[FR-024-list-filter-search-and-sort-pieces]]"
  - "[[FR-025-register-as-an-artist]]"
  - "[[FR-026-sign-the-artist-contract]]"
tags: [stakeholder]
aliases: [Head Office, CEO, TSU management]
role: Business owner / Head Office
influence: high
interest: high
provides: []
receives: []
nature: person
context_role: Head Office
---

# Gus Renoir

**Snapshot.** Runs the business out of the Head Office in New York, showing
the work of up-and-coming artists found by the company's founder (the
"Creative Genius"). Owns the tour concept end-to-end: negotiating artist and
lending agreements, arranging shipping/insurance, and overseeing sales across
every gallery on a tour. Commissioned this digitization effort.

**Goals.** [[PAM]] (light)
- Replace ad-hoc, memory-dependent tour coordination with a system that
  tracks every piece's status (on loan, for sale, sold, shipped) across all
  tour stops.
- Keep commission, deposit, and payment-deadline rules enforced consistently,
  without manual tracking per buyer.

**Concerns / pains.**
- Losing track of which pieces are for sale vs. on loan (from artists,
  private owners, or museums) as a tour moves between galleries.
- Missed deadlines in the buyer payment sequence (10% within 24h/until
  Monday, remaining 90% within 14 days) going uncaught.
- Coordinating one shipper and one insurer across an entire multi-stop tour.

**Interactions.** No [[UC-...]] or [[FR-...]] captured yet — pending the
`ingest.md` pass over `raw/TSU-brief.md.md`.

> [!note] Also listed as "CEO / TSU management"
> The Req4Arc Linz workshop
> ([[SRC-002-georg-mayrhauser-vision-goals-stakeholders]]) opened its stakeholder
> list with "CEO / TSU management" rather than a name. Merged here as aliases
> rather than duplicated — but note the workshop framing is *organizational*
> (a management layer) while the brief's framing is one named owner. If TSU has
> a management team distinct from Gus Renoir, this page needs to split.
